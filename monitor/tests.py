...
class TestCreateMeasurementAPI(TestCase):
    ...
    def test_create_measurement_api(self):
        response = self.client.post(reverse('measurement'), data=self.data, format='json')
...
class TestUpdateMeasurementAPI(TestCase):
    ...
    def test_update_error(self):
        ...
        response = client.put(reverse('measurement-detail', kwargs={'pk': measurement.id}), data=self.data,
                                        format='json'
                                        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST, response.data)
